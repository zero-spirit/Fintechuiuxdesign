...
// Generate mock historical data based on current price with fallback
const generateHistoricalData = (currentPrice?: number, changePercent?: number): HistoricalDataPoint[] => {
  ...
  const basePrice = safePrice / (1 + safeChange / 100);

  for (let i = 0; i < periods; i++) {
    const volatility = 0.02; // 2% volatility
    const trend = (safeChange / 100) / periods; // Distribute the change across periods
  ...
  data[data.length - 1].price = safePrice;
            ...
            <div className="relative bg-gradient-to-br from-card/95 to-card/90 backdrop-blur-xl border border-border/50 rounded-2xl shadow-2xl">
              ...
              <div className="relative p-6 md:p-8">
                ...
                <div className="flex items-start justify-between mb-6">
                  <div className="flex items-center gap-4">
                    <div className={`w-16 h-16 rounded-xl bg-gradient-to-br ${
                      ...
                      {(stock.symbol || 'N/A').substring(0, 2)}
                    </div>
                    <div>
                      <h2 className="text-2xl font-bold mb-1">{stock.symbol || 'Unknown'}</h2>
                      <p className="text-muted-foreground">{stock.name || 'Unknown Stock'}</p>
                ...
                <div className="mb-6">
                  ...
                  <div className={`flex items-center gap-2 ${getChangeColor(change)}`}>
                    {isPositive ? (
